...
public class RegisterEvent extends JFrame implements ActionListener {
    ...
    public RegisterEvent() {
        ...
        panel.setBorder(new TitledBorder(new LineBorder(new Color(47, 79, 79), 2, true), "Register-Event",
        ...
        try {
            ...
            if (ae.getSource() == b1) {
                ...
                while (rs.next()) {
                    ...
                    t3.setText(rs.getString("start_date"));
            ...
            if (ae.getSource() == b3) {
                ///
                String sqls = "select * from registerevent where event_id = ? and student_id = ?";
                PreparedStatement sts = con.c.prepareStatement(sqls);
                sts.setString(1, t1.getText());
                sts.setString(2, t8.getText());
                ResultSet rss = sts.executeQuery();

                if (rss.next()) {
                    JOptionPane.showMessageDialog(null, "Student already registered for Event!.");
                } else {
                    try {
                        String sql = "insert into registerevent(event_id, student_id, ename, sname, course, branch, price, dateOfRegister) values(?, ?, ?, ?, ?, ?, ?, ?)";
                        PreparedStatement st = con.c.prepareStatement(sql);
                        st.setString(1, t1.getText());
                        ...
                        if (i > 0) {
                            JOptionPane.showMessageDialog(null, "Successfully Register For Event.!");
                        } else {
